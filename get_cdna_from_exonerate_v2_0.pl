...
#		print @true_postions_array[1..3],"\n\n";
		...
		if($#scores >= 1){
		print OUT_FINAL_BED join("\t",@true_postions_array[1..3]),"\t",$N_percentages[$idxMax],"\t",$scores[$idxMax],"\t",$inlines[6],"\t",$TEs[$idxMax],"\t",$sequences[$idxMax],"\t",$scores_all,"\n";
		}
	}
}
...
#                                   -desc => $fasta_desc);
		 my $seq_obj_bed = Bio::Seq->new ( 
											  -seq => $sequence_fasta[-1],
                                   -id  => $inline_bed[6].".".$te_num{$inline_bed[6]}.".".$fasta_desc,
                                   -accession_number => 'TE_'.$i,
                                   -alphabet => 'dna',
                                   				);
       my $seq_obj_bed_2 = Bio::Seq->new ( 
...
#				print $seq_obj_bed_2->seq();
			$seq = substr $seq_obj_bed->seq(),0,$where*3;
